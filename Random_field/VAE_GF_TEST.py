...
dataset = torchvision.datasets.ImageFolder('C:/Users/farideh/Desktop/Rnadom field/VAE and RF/Test_data',
                                           transforms.Compose([
...
class VariationalAutoencoder(nn.Module):
    ...
    def latent_sample(self, eigenvalues):
        ...
        if self.training:
            ...
            # sample_a = np.random.multivariate_normal(torch.zeros(1), torch.eye(1), (batch_size, latent_dims, 1))
            # sample_b = np.random.multivariate_normal(torch.zeros(1), torch.eye(1), (batch_size, latent_dims, 1))
            #
            # zeta.real = torch.tensor(sample_a.reshape(batch_size, latent_dims, 1))
            # zeta.imag = torch.tensor(sample_b.reshape(batch_size, latent_dims, 1))

            L = torch.matmul(DFT_Mat_conj, torch.square(torch.diag_embed(eigenvalues)).cfloat()).real
            #z = torch.matmul(L, esp.cfloat()).real
...
for epoch in range(num_epochs):
    for batch_idx, (image_batch, _) in enumerate(data_loader):
        ...
        # axeigen.plot(min)
        # axeigen.plot(median)
        # figeig.savefig(str(i)+'eigen_VAE_MRI_W.png')
        ...
        axs[1, 4].grid(False)
        plt.grid(b=None)
        figs.savefig(str(i)+'sample.png')

        plt.close()

        figlatenet.savefig(str(i)+'latent.png')
        ...
        figcov.savefig(str(i)+"COVARIANCE.png")
        plt.close()
        print("one batch")
        i += 1
